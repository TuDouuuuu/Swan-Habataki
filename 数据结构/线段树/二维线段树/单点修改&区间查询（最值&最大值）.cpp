...
void subUpdate(int xrt, int y, int c, int l, int r, int rt) {
    if(l == r){
        if(leaf){
            mp[xrt][rt]+=c;//mp[xrt][rt]=c;
        }
        else{
            mp[xrt][rt]=mp[xrt<<1][rt]+mp[xrt<<1|1][rt];//mp[xrt][rt]=max(mp[xrt<<1][rt],mp[xrt<<1|1][rt]);
        }
        return ;
    }
    else {
        ...
        mp[xrt][rt]=mp[xrt][rt<<1]+mp[xrt][rt<<1|1];//mp[xrt][rt]=max(mp[xrt][rt<<1],mp[xrt][rt<<1|1]);
    }
}
...
int subQuery(int xrt, int yl, int yr, int l, int r, int rt) {
    if(yl <= l && r <= yr) return mp[xrt][rt];
        ...
        int m = l + r >> 1;
        int res=0;//int res=-1;
        if(yl <= m) res += subQuery(xrt, yl, yr, lson);//res=max(res,subQuery(xrt, yl, yr, lson));
        if(yr > m) res += subQuery(xrt, yl, yr, rson);//res=max(res,subQuery(xrt, yl, yr, rson));
        return res;
    }
}
int query(int xl, int xr, int yl, int yr, int l, int r, int rt) {//xl<=xr,yl<=yr
    if(xl <= l && r <= xr) return subQuery(rt, yl, yr, 0, n, 1);
        ...
        int m = l + r >> 1;
        int res = 0;//int res=-1;
        if(xl <= m) res += query(xl, xr, yl, yr, lson);//res=max(res,query(xl, xr, yl, yr, lson));
        if(xr > m) res += query(xl, xr, yl, yr, rson);//res=max(res,query(xl, xr, yl, yr, rson))
        return res;
    }
}
