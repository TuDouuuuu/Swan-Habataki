...
### 稳定婚姻匹配：Gale-Shapley算法

模板题：

有 $n1$ 男 $n2$ 女，均为异性恋，每个人都对异性有好感度排序。能否给出稳定的牵手方案？

稳定的定义：如果男孩 $u_1$ 和女孩 $v_1$ 牵手，男孩 $u_2$ 和女孩 $v_2$ 牵手。但男孩 $u_1$ 相比 $v_1$ 对 $v_2$ 更喜欢，而女孩 $v_2$ 相比 $u_2$ 对 $u_1$ 更喜欢，那么 $u_1$ 会和 $v_2$ 私奔，这即是不稳定的。
...
1. 假设把匹配看成边，实现的复杂度为 $O(n+m)$ 。如果存在多重匹配，可以用优先队列维护。
2. 该算法肯定能够得到稳定的婚姻。
3. 算法匹配的结果不唯一。
4. 假设每个人对一些异性有好感，这种好感不分大小。要使尽量多对互有好感的人相匹配，问题就变成了二分图最大匹配。
...
以上两行在放屁，只能说随机数据命中率较高，构造十字对称的基环树能轻松卡掉。

（估了一下，随机 $\sqrt{n}$ 个不同起点做两次 bfs ，命中答案的期望是1）

![graph-dia](F:\ACM\Hartley的ACM板子\LaTeX\ACM模板\graph-dia.png)
...
推论 2：Dirac定理。设 $G$ 是 $n(n\ge3)$ 的无向简单图，若对于 $G$ 中任意顶点 $v_i$ ，均有 $d(v_i)\ge\lceil\frac{n}{2}\rceil$ ，则 $G$ 中存在哈密顿回路，从而 $G$ 为哈密顿图。

#### 已知满足Dirac定理，求一条哈密顿回路：$O(n^2 )$
...
求回路只要保证通路的同时首尾连通，枚举 $n$ 个点作为起点找通路，找到就 break，复杂度 $O(n^3)$。
...
网上有一些论文提出了 $O(V+E)=O(3*V)$ 的线性做法，但是没有实现的好的代码。以下代码参考（https://github.com/Keksozavr/graph_planarity）并进行了修改，理论上是线性的（把重边预处理的set换掉的话），然而常数巨大，空间也巨大，建议 $n\le3e5$ 的图可以尝试。
...
struct gap {
	pii low, high;
	gap(pii a = None, pii b = None): low(a), high(b) {}
	int empty() {
		return low == None && high == None;
	}
	bool operator==(const gap &p) {
		return low == p.low && high == p.high;
	}
};
struct cpair {
	gap L, R;
	cpair(gap l = gap(), gap r = gap()): L(l), R(r) {}
...
int pid[MAXN];
int Fa[MAXN], dep[MAXN];
vi low[MAXN], low2[MAXN];
vi ew[MAXN];
vector<pii>low_e[MAXN];
vector<cpair>bot[MAXN];
vi side[MAXN];
vector<pii>reff[MAXN];
void dfs1(int u, int fa) {
	Fa[u] = fa;
	dep[u] = dep[fa] + 1;
	for(auto v: e1[u]) {
...
void dfs2(int u, int fa) {
	...
	for(int i = 0; i < SZ(e2[u]); i++) {
		...
		if(Fa[v] == u) { // son
			...
			low[u][i] = dep[v];
		ew[u][i] = 2 * low[u][i] + (low2[u][i] < dep[u]);
		if(fa != 0) {
			int p = pid[u];
			if(low[u][i] < low[fa][p]) {
				low2[fa][p] = low[fa][p];
				low[fa][p] = low[u][i];
			} else if(low[u][i] > low[fa][p])
				low2[fa][p] = min(low2[fa][p], low[u][i]);
			else
				low2[fa][p] = min(low2[fa][p], low2[u][i]);
		}
	}
}
int& Low(pii x) {
	return low[x.fi][x.se];
}
int& Side(pii x) {
	return side[x.fi][x.se];
}
int& E2(pii x) {
	return e2[x.fi][x.se];
}
pii& Ref(pii x) {
	return reff[x.fi][x.se];
}
pii& Low_e(pii x) {
	return low_e[x.fi][x.se];
}
cpair& Bot(pii x) {
	return bot[x.fi][x.se];
}
int lowest(cpair x) {
	if(x.L.empty())
		return Low(x.R.low);
	if(x.R.empty())
		return Low(x.L.low);
	return min(Low(x.L.low), Low(x.R.low));
}
bool conf(gap x, pii e) {
	return !x.empty() && Low(x.high) > Low(e);
}
cpair stk[MAXN];
int top = 0;
bool add_cons(pii u_v, pii fa_u) {
	cpair x;
	while(true) {
		cpair y = stk[top--];
		if(!y.L.empty())
		...
		if(!y.L.empty())
			return false;
		if(Low(y.R.low) > Low(fa_u)) {
			if(x.R.empty())
				x.R = y.R;
			else
				Ref(x.R.low) = y.R.high;
			x.R.low = y.R.low;
		} else
			Ref(y.R.low) = Low_e(fa_u);
		if(top > 0 && stk[top] == Bot(u_v))
			break;
	}
	while(conf(stk[top].L, u_v) || conf(stk[top].R, u_v)) {
		cpair y = stk[top--];
		if(conf(y.R, u_v))
			swap(y.L, y.R);
		if(conf(y.R, u_v))
		...
		if(x.L.empty())
			...
			Ref(x.L.low) = y.L.high;
		x.L.low = y.L.low;
	}
	if(!x.L.empty() || !x.R.empty()) 
		stk[++top] = x;
	return true;
}
#define piii pair<int, pii>
vector<piii> barrel[MAXN * 2];
vector<pii> e3[MAXN];
bool check_planarity(int u) {
	int fa = Fa[u];
	for(auto i: e3[u]) {
		int v = i.fi, p = i.se;
		pii a = {u, p}, b = {fa, pid[u]};
		if(top > 0)
			bot[u][p] = stk[top];
		if(u == Fa[v]) {
			if(!check_planarity(v))
				return false;
		} else {
			low_e[u][p] = {u, p};
			stk[++top] = cpair(gap(a, a));
		}
		if(low[u][p] < dep[u] && fa != 0) {
			if(v == e3[u][0].fi)
				low_e[fa][pid[u]] = low_e[u][p];
			else if(!add_cons(a, b)) 
				return false;
		}
	}
	if(fa != 0) {
		while(top > 0 && lowest(stk[top]) == dep[fa]) {
			cpair x = stk[top--];
			if(!x.L.empty() && x.L.low != None)
				Side(x.L.low) = 0;
		}
		if(top > 0) {
			cpair x = stk[top--];
			while(x.L.high != None && E2(x.L.high) == fa) {
				if(Ref(x.L.high) != None)
					x.L.high = Ref(x.L.high);
				else
					x.L.high = None;
			}
			if(x.L.high == None && x.L.low != None) {
				Ref(x.L.low) = x.R.low;
				Side(x.L.low) = 0;
				x.L.low = None;
			}
			while(x.R.high != None && E2(x.R.high) == fa) {
				if(Ref(x.R.high) != None)
					x.R.high = Ref(x.R.high);
				else
					x.R.high = None;
			}
			if(x.R.high == None && x.R.low != None) {
				Ref(x.R.low) = x.L.low;
				Side(x.R.low) = 0;
				x.R.low = None;
			}
		...
		if(low[fa][pid[u]] < dep[fa]) {
			pii x = stk[top].L.high, y = stk[top].R.high;
			if(x != None && (y == None || Low(x) > Low(y)))
				reff[fa][pid[u]] = x;
			else
				reff[fa][pid[u]] = y;
		}
